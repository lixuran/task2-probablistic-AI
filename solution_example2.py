...
class DropoutTrainer(Framework):
    ...
    def train(self):
        ...
        for _ in progress_bar:
            for batch_idx, (batch_x, batch_y) in enumerate(self.train_loader):
                ...
                current_logits = self.network(batch_x)
                loss_1 = F.nll_loss(F.log_softmax(current_logits, dim=1), batch_y, reduction='mean')/self.tau
                C = self.l*self.l/(2*self.tau*self.batch_size)
                loss_2 = C*(self.dropout_p*(self.network.layer1.weight**2).sum()+(self.network.layer1.bias**2).sum()+
                            self.dropout_p*(self.network.layer2.weight**2).sum()+(self.network.layer2.bias**2).sum()+
                            self.dropout_p*(self.network.layer3.weight**2).sum()+(self.network.layer3.bias**2).sum()+
                            self.dropout_p*(self.network.layer4.weight**2).sum()+(self.network.layer4.bias**2).sum())
    ...
    def predict_probabilities(self, x: torch.Tensor, num_sample=100) -> torch.Tensor:
        ...
        for i in range(self.num_test-1):
            #enable_dropout(self.network)
            output = output+F.softmax(self.network(x), dim=1)
    
        estimated_probability = output/self.num_test
