    ...
    def train(self):
        ...
        for _ in progress_bar:
            for batch_idx, (batch_x, batch_y) in enumerate(self.train_loader):
                ...
                current_logits = self.network(batch_x)
                loss_first_term = F.nll_loss(F.log_softmax(current_logits, dim=1), batch_y, reduction='sum') / (self.tau*self.batch_size)
                def compute_layer_loss(layer):
                    layer_loss = (layer.weight**2).sum()*(self.dropout_p*self.length_scale/(2*self.tau*self.batch_size ))
                    layer_loss = layer_loss + (layer.bias**2).sum()*(self.length_scale/(2*self.tau*self.batch_size ))
                    return layer_loss
                loss_second_term = compute_layer_loss(self.network.layer1) + compute_layer_loss(self.network.layer2)
