...
class DropoutTrainer(Framework):
    def __init__(self, dataset_train,
        ...
        self.l = 0.3
        self.tau = 1
        self.c = self.l**2/(2*self.tau*self.batch_size)
        ...
        # self.network = None
        # self.optimizer = None
        self.network = MNISTNet(in_features=28*28,out_features=10,dropout_p=self.p,dropout_at_eval=True)
        self.train_loader = torch.utils.data.DataLoader(
            dataset_train, batch_size=self.batch_size, shuffle=True, drop_last=True
            )
        self.optimizer = torch.optim.Adam(self.network.parameters(), lr=self.learning_rate   ) 
    ...
    def train(self):
        ...
        for _ in progress_bar:
            for batch_idx, (batch_x, batch_y) in enumerate(self.train_loader):
                ...
                loss = loss+(self.network.layer1.weight**2).sum()*self.p*self.c + (self.network.layer1.bias**2).sum()*self.c +(self.network.layer2.weight**2).sum()*self.p*self.c + (self.network.layer2.bias**2).sum()*self.c 
    ...
    def predict_probabilities(self, x: torch.Tensor, num_sample=100) -> torch.Tensor:
        ...
        #     estimated_probabilitys = [F.softmax(self.network(x), dim=1) for _ in range(num_sample)]
        #     estimated_probability = torch.stack(estimated_probabilitys,dim=0)
        #     estimated_probability = torch.mean(estimated_probability,dim=0)
        #     print(num_sample,estimated_probabilitys.shape,"shit")
        # assert estimated_probability.shape == (x.shape[0], 10)  
        estimated_probability = torch.zeros(10)
        for i in range(self.num_simulations):
            estimated_probability = estimated_probability + F.softmax(self.network(x), dim=1)
        estimated_probability = estimated_probability / self.num_simulations
...
class EnsembleTrainer(Framework):
    def __init__(self, dataset_train,
        ...
        self.EnsembleNetworks = [
            MNISTNet(in_features=28*28,out_features=10,dropout_p=0.1)
            for _ in range(self.num_heads)
        ]
        self.optimizers = [torch.optim.Adam(self.EnsembleNetworks[i].parameters(), lr=self.learning_rate,weight_decay=0   ) for i in range(self.num_heads)]
        self.train_loader = torch.utils.data.DataLoader(
            dataset_train, batch_size=self.batch_size, shuffle=True, drop_last=True
            )
    ...
    def train(self):
        ...
        for net_idx, network in enumerate(self.EnsembleNetworks):   
            network.train()
            progress_bar = trange(self.num_epochs)
            #print("whats going on1")
            for _ in progress_bar:
                for batch_idx, (batch_x, batch_y) in enumerate(self.train_loader):
                    # batch_x are of shape (batch_size, 784), batch_y are of shape (batch_size,)
                    #print(batch_x.max(),batch_x.min())
                    network.zero_grad()
                    if batch_x.requires_grad == True:
                        batch_x.zero_grad()
                    ...
                    # Backpropagate to get the gradients
                    loss.backward()
                   # print(batch_x.grad.shape)
                    #grad = torch.autograd.grad(loss, input, retain_graph=True)[0].data
                    batch_x_adv = batch_x+ 2.55*torch.sign(batch_x.grad)
                    current_logits = network(batch_x_adv)
                    loss2 = F.nll_loss(F.log_softmax(current_logits, dim=1), batch_y, reduction='mean')
                    loss2.backward()
                    self.optimizers[net_idx].step()
    ...
    def predict_probabilities(self, x: torch.Tensor) -> torch.Tensor:
        ...
        estimated_probability /= self.num_heads
        assert estimated_probability.shape == (x.shape[0], 10)  
        return estimated_probability
